...
class AndroidVideoPlayer extends VideoPlayerPlatform {
  ...
  /// LOCAL ADDITION — data source uri → player id, for
  /// [setAudioEnabledForSource]. Kept in step with [_players]: written in
  /// createWithOptions, cleared in dispose.
  final Map<String, int> _playerIdsBySource = <String, int>{};
  ...
  Future<void> dispose(int playerId) async {
    ...
    // LOCAL ADDITION — see setAudioEnabledForSource. Removed by value, not by
    // a remembered key: two players can be opened for the same uri in
    // sequence, and the map holds only the most recent, so deleting by a
    // stale key would drop the live player's entry.
    _playerIdsBySource.removeWhere((_, int id) => id == playerId);
  ...
  Future<int?> createWithOptions(VideoCreationOptions options) async {
    ...
    // LOCAL ADDITION — see setAudioEnabledForSource.
    _playerIdsBySource[uri] = playerId;
  ...
  /// Enables or disables audio decoding entirely for the player playing
  /// [sourceUri], returning false if no such player is open.
  ///
  /// LOCAL ADDITION — not part of [VideoPlayerPlatform], so it has no
  /// `@override` and callers reach it through a cast to this class. See
  /// LOCAL_CHANGES.md at the root of this package.
  ///
  /// This is not `setVolume(0)`. Volume silences the output while the decoder
  /// keeps running; this stops the work, releasing the hardware audio decoder
  /// and the AudioTrack that go with it. Intended for players kept warm off
  /// screen, which decode sound nobody can hear.
  ///
  /// Addressed by SOURCE rather than by player id because the id is reachable
  /// from an app only through `VideoPlayerController.playerId`, which upstream
  /// marks `@visibleForTesting` and documents as "shouldn't be used by anyone
  /// depending on the plugin". `VideoPlayerController.dataSource` is public and
  /// is the same string this class receives as the uri, so it addresses a
  /// player without reaching past anybody's contract.
  ///
  /// The bool is the honest half of that trade: matching on a string can go
  /// quietly wrong in a way an integer id cannot, so an unknown source is
  /// reported rather than silently ignored, and the caller can assert on it.
  Future<bool> setAudioEnabledForSource(
    String sourceUri, {
    required bool enabled,
  }) async {
    final int? playerId = _playerIdsBySource[sourceUri];
    if (playerId == null) {
      return false;
    }
    await _playerWith(id: playerId).setAudioEnabled(enabled: enabled);
    return true;
  }
...
class _PlayerInstance {
  ...
  Future<void> setAudioEnabled({required bool enabled}) {
    return _api.setAudioEnabled(enabled);
  }
